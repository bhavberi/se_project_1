...
public class TestUserResource {
    ...
    public void testUserResource() throws JSONException {
        ...
        WebResource userResource = baseJerseyTest.resource().path("/user");
        ...
        // Create alice user
        baseJerseyTest.clientUtil.createUser("alice");

        // Login admin
        String adminAuthenticationToken = baseJerseyTest.clientUtil.login("admin", "admin", false);

        // List all users
        userResource = baseJerseyTest.resource().path("/user/list");
        ...
        postParams.putSingle("email", "bob@books.com");
        ...
        postParams = new MultivaluedMapImpl();
        postParams.putSingle("username", "bob");
        postParams.putSingle("email", " bobbooks.com ");
        postParams.putSingle("password", "12345678");
        response = userResource.put(ClientResponse.class, postParams);
        Assert.assertEquals(Status.BAD_REQUEST, Status.fromStatusCode(response.getStatus()));
        json = response.getEntity(JSONObject.class);
        ...
        postParams.putSingle("email", " bob@books.com ");
        postParams.putSingle("password", " 12345678 ");
        postParams.putSingle("locale", "en");
        response = userResource.put(ClientResponse.class, postParams);
        Assert.assertEquals(Status.OK, Status.fromStatusCode(response.getStatus()));
        ...
        userResource = baseJerseyTest.resource().path("/user/check_username");
        ...
        userResource = baseJerseyTest.resource().path("/user/login");
        postParams = new MultivaluedMapImpl();
        postParams.putSingle("username", " alice ");
        postParams.putSingle("password", " 12345678 ");
        response = userResource.post(ClientResponse.class, postParams);
        Assert.assertEquals(Status.OK, Status.fromStatusCode(response.getStatus()));
        String aliceAuthToken = baseJerseyTest.clientUtil.getAuthenticationCookie(response);

        // Login user bob twice
        String bobAuthToken = baseJerseyTest.clientUtil.login("bob");
        String bobAuthToken2 = baseJerseyTest.clientUtil.login("bob");

        // List sessions
        userResource = baseJerseyTest.resource().path("/user/session");
        ...
        Assert.assertEquals(Status.OK, Status.fromStatusCode(response.getStatus()));
        json = response.getEntity(JSONObject.class);
        Assert.assertEquals("alice@books.com", json.getString("email"));
        ...
        userResource.addFilter(new CookieAuthenticationFilter(aliceAuthToken));
        postParams = new MultivaluedMapImpl();
        postParams.add("email", " alice2@books.com ");
    ...
    public void testUserResourceAdmin() throws JSONException {
        ...
        baseJerseyTest.clientUtil.createUser("admin_user1");
        ...
        // User admin deletes himself: forbidden
        userResource = baseJerseyTest.resource().path("/user/admin");
        userResource.addFilter(new CookieAuthenticationFilter(adminAuthenticationToken));
        response = userResource.delete(ClientResponse.class);
        Assert.assertEquals(Status.BAD_REQUEST, Status.fromStatusCode(response.getStatus()));
        json = response.getEntity(JSONObject.class);
        ...
        userResource = baseJerseyTest.resource().path("/user/admin_user1");
        userResource.addFilter(new CookieAuthenticationFilter(adminAuthenticationToken));
        response = userResource.delete(ClientResponse.class);
        Assert.assertEquals(Status.OK, Status.fromStatusCode(response.getStatus()));
        json = response.getEntity(JSONObject.class);
